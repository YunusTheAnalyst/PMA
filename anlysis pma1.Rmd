...
### Splitting the data
70-30 percent cross validation techniques was used to split the data into train and and test sets
This cross validation sets up for the highest prediction accuracy.
```{r}
...
```{r}
score_data<- data.frame(link=prediction2,
                        response=prediction1,
                        fpuse=Final_FP_test$fp_use,
                        stringsAsFactors = FALSE)
library(ggplot2)
library(pROC)
score_data %>% 
  ggplot(aes(x=link, y=response, col=fpuse))+
  scale_color_manual(values = c("black", "red"))+
  geom_point()+
  geom_rug()+
  ggtitle("Scores")
auc(Final_FP_test$fp_use, prediction2)
```
...
plot(roc(Final_FP_test$fp_use, prediction2, direction="<"), col="yellow", lwd=3, main="Sensitivity vs Specificity")
glm_simple_roc<-simple_roc(Final_FP_test$fp_use=="TRUE",prediction2)
